...
func _ready() -> void:
	...
	var training_ground := _gameplay_runtime.get_node_or_null("World") as Node3D if _gameplay_runtime != null else null
	if _gameplay_runtime == null or training_ground == null:
		push_error("TrainingGroundPlaytest requires its gameplay runtime and training World.")
		return
