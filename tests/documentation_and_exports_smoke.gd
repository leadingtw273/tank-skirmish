...
const SCRIPT_REQUIREMENTS := {
	...
	"res://src/actors/tank/tank_controller.gd": ["signal shot_fired", "signal shot_event_fired", "func set_movement_input", "func set_turn_input", "func aim_turret_at", "func aim_gun_pitch_at_target", "func muzzle_global_position", "func muzzle_global_direction", "func request_fire"],
}

const DEFAULT_REQUIREMENTS := {
	"res://src/player/aim_presentation.gd": ["@export var max_aim_distance := 180.0", "@export_flags_3d_physics var aim_collision_mask := 129", "@export var aim_line_radius := 0.04", "@export var aim_line_min_length := 0.05", "@export var aim_line_near_tank_hidden_distance := 3.0", "@export_range(0.0, 1.0, 0.05) var aim_line_alpha := 0.7", "@export var aim_aligned_angle_radians := 0.004363323"],
	...
	"res://src/actors/tank/tank_controller.gd": ["@export var movement_speed := 15.0", "@export var turn_speed := 0.8", "@export_range(0.0, 1.0, 0.05) var turning_movement_speed_ratio := 0.5", "@export_range(0.0, 1.0, 0.05) var firing_movement_speed_loss_ratio := 0.25", "@export var tank_mass_tonnes := 60.0", "@export var engine_horsepower := 1500.0", "@export var brake_force_kilonewtons := 240.0", "@export var turn_response := 0.4", "@export var tread_animation_blend_seconds := 0.12", "@export_range(0.01, 100.0, 0.01) var tread_animation_reference_speed := 15.0", "@export_range(0.0, 4.0, 0.01) var tread_animation_speed_multiplier := 1.0", "@export var turret_turn_speed := 1.777778", "@export var gun_pitch_speed := 1.2", "@export_range(0.0, 45.0, 0.5) var gun_max_elevation_degrees := 20.0", "@export_range(0.0, 45.0, 0.5) var gun_max_depression_degrees := 8.0", "@export var visual_recoil_distance := 0.36", "@export var visual_recoil_kick_seconds := 0.04", "@export var visual_recoil_return_seconds := 0.18", "@export var max_camera_look_ahead_distance := 30.0", "@export var muzzle_flash_lifetime_seconds := 0.25", "@export var muzzle_flash_scale := 4.0", "@export_range(0.1, 4.0, 0.05) var tread_dust_scale := 0.85", "@export_range(0.1, 5.0, 0.05) var tread_dust_lifetime_seconds := 1.2", "@export_range(2, 128, 1) var tread_dust_emission_amount := 28", "@export_range(0.01, 5.0, 0.01) var tread_dust_activation_speed := 0.15"],
}
